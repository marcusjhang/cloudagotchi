...
```
┌─────────────────────────┐          ┌──────────────────────────────────┐
│   ESP32-S3 (the body)   │   MQTT   │            AWS (the brain)       │
│                         │◄────────►│                                  │
│  LVGL pet on AMOLED     │ IoT Core │  Lambda ─── DynamoDB (pet state) │
│  Touch → interactions   │          │    ▲                             │
│  IMU   → shake/pickup   │          │  EventBridge (time passes)       │
│  Speaker ← briefings    │          │  Bedrock + Polly (the news)      │
└─────────────────────────┘          └──────────────────────────────────┘
```
