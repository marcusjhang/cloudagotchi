 ...
 * bars, one face sprite, one (optional) news badge. The face only changes
...
static lv_timer_t *s_talk_timer;
...
static void blink_close_cb(lv_timer_t *t)
{
    ...
    if (!s_talking && !s_dozing) set_face(mood_sprite(s_mood));
    lv_timer_delete(t);
}
...
{
    if (s_talking) return;      // reading the news is not boredom
    s_dozing = true;
...
static void note_activity(void)
{
    ...
    if (s_dozing) {
        s_dozing = false;
        if (!s_talking) set_face(mood_sprite(s_mood));
    }
}

/* ---------- talking: three-frame cycle ----------------------------------- */

static void talk_timer_cb(lv_timer_t *t)
{
    static const lv_image_dsc_t *cycle[] =
        { &face_talk_1, &face_talk_2, &face_talk_3, &face_talk_2 };
    static int i;
    i = (i + 1) % 4;
    set_face(cycle[i]);
}
...
void pet_ui_set_state(uint8_t hunger, uint8_t energy, uint8_t mood_value, pet_mood_t mood)
{
    ...
    if (!s_talking && !s_dozing) set_face(mood_sprite(mood));
    bsp_display_unlock();
}

/* ---------- the morning paper -------------------------------------------*/

static lv_obj_t *s_briefing_badge;
static lv_obj_t *s_briefing_label;
static pet_briefing_cb_t s_on_read;

static void briefing_clicked_cb(lv_event_t *e)
{
    if (s_on_read) s_on_read();
}

void pet_ui_show_briefing(const char *headline, pet_briefing_cb_t on_read)
{
    bsp_display_lock(0);
    note_activity(); // the paperboy wakes the pet
    s_on_read = on_read;

    if (s_briefing_badge == NULL) {
        s_briefing_badge = lv_obj_create(lv_screen_active());
        lv_obj_set_size(s_briefing_badge, 330, 72);
        lv_obj_align(s_briefing_badge, LV_ALIGN_BOTTOM_MID, 0, -14);
        lv_obj_set_style_bg_color(s_briefing_badge, lv_color_hex(0xFF9900), 0);
        lv_obj_set_style_radius(s_briefing_badge, 36, 0);
        lv_obj_clear_flag(s_briefing_badge, LV_OBJ_FLAG_SCROLLABLE);
        lv_obj_add_flag(s_briefing_badge, LV_OBJ_FLAG_CLICKABLE);
        lv_obj_add_event_cb(s_briefing_badge, briefing_clicked_cb, LV_EVENT_CLICKED, NULL);

        s_briefing_label = lv_label_create(s_briefing_badge);
        lv_label_set_long_mode(s_briefing_label, LV_LABEL_LONG_DOT); // no marquee
        lv_obj_set_width(s_briefing_label, 290);
        lv_obj_set_style_text_color(s_briefing_label, lv_color_hex(0x232F3E), 0);
        lv_obj_center(s_briefing_label);
    }
    lv_label_set_text_fmt(s_briefing_label, "News! %s", headline);
    lv_obj_clear_flag(s_briefing_badge, LV_OBJ_FLAG_HIDDEN);
    bsp_display_unlock();
}
...
{
    bsp_display_lock(0);
    if (s_briefing_badge) lv_obj_add_flag(s_briefing_badge, LV_OBJ_FLAG_HIDDEN);
    bsp_display_unlock();
}

/* ---------- quiet mode (while the pet reads aloud) -----------------------*/
...
void pet_ui_quiet_mode(bool enable)
{
    ...
    s_talking = enable;
    if (enable) {
        if (s_blink_timer) lv_timer_pause(s_blink_timer);
        if (s_talk_timer == NULL) {
            s_talk_timer = lv_timer_create(talk_timer_cb, 200, NULL);
        }
        lv_timer_resume(s_talk_timer);
    } else {
        if (s_talk_timer) lv_timer_pause(s_talk_timer);
        if (s_blink_timer) lv_timer_resume(s_blink_timer);
